...
// Pacotes Comuns
enum{
    FXW_CMD_DEVICE_ID_L = 0x00,
    FXW_CMD_DEVICE_ID_H,
    FXW_CMD_LOT_L,
    FXW_CMD_LOT_H,
    FXW_CMD_LOT_DATE_L,
    FXW_CMD_LOT_DATE_H,
    FXW_CMD_FOXWIRE_VERSION_ID,
    FXW_CMD_FIRMWARE_ID,
    FXW_CMD_FIRMWARE_VERSION,
    FXW_CMD_REQUEST_WRITE,
    FXW_CMD_MCU_RESET,
    FXW_CMD_MCU_VOLTAGE,
    FXW_CMD_MCU_TEMPERATURE
};

// Comandos com chave comuns
enum{
    FXW_CMD_W_SAVE = 0x01,
    FXW_CMD_W_RESTORE,
    FXW_CMD_W_RESTORE_KEPP_ADDR
};
//---------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------
// Inplementação dos PACOTES  READ e WRITE
// FOX READ
template< const uint8_t pin >
uint8_t FoxWire_READ(uint8_t addr, uint8_t data) {
  cli();
...
// FOX WRITE
template< const uint8_t pin >
uint8_t FoxWire_WRITE(uint8_t addr, uint8_t data1, uint8_t data2 ) {
  cli();
...
//---------------------------------------------------------------------------------
// High level
#define FXW_CHECKSUM( data )  ( 0x3&( (data&1) + ((data>>1)&1) + ((data>>2)&1) + ((data>>3)&1) + ((data>>4)&1) ) )
// Read register
template< const uint8_t pin >
uint8_t FoxWire_register_read(uint8_t addr, uint8_t reg_addr) {
  reg_addr &= 0x1F;
  return FoxWire_READ<pin>(addr, 0x80 | (FXW_CHECKSUM(reg_addr)<<5) | reg_addr );
}
// Write register
template< const uint8_t pin >
uint8_t FoxWire_register_write(uint8_t addr, uint8_t reg_addr, uint8_t value ) {
  reg_addr &= 0x1F;
  return FoxWire_WRITE<pin>(addr, 0x80 | (FXW_CHECKSUM(reg_addr)<<5) | reg_addr, value );
}
// command READ or WRITE
template< const uint8_t pin >
uint8_t FoxWire_command(uint8_t addr, uint8_t cmd) {
  cmd &= 0x1F;
  return FoxWire_READ<pin>(addr, (FXW_CHECKSUM(cmd)<<5) | cmd );
}
template< const uint8_t pin >
uint8_t FoxWire_command(uint8_t addr, uint8_t cmd, uint8_t arg) {
  cmd &= 0x1F;
  return FoxWire_WRITE<pin>(addr, (FXW_CHECKSUM(cmd)<<5) | cmd, arg );
}
// command with key
template< const uint8_t pin >
uint8_t FoxWire_command_key(uint8_t addr, uint8_t cmd) {
  uint8_t KEY = FoxWire_command<pin>( addr, FXW_CMD_REQUEST_WRITE );
  return FoxWire_command<pin>( addr, cmd, ~KEY );
}
//---------------------------------------------------------------------------------
